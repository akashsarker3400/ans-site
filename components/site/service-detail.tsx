...
import { PRICING_BN, SERVICE_BN, SERVICES, type Locale, type Service } from "@/lib/content";
...
export function ServiceDetail({ service, locale }: { service: Service; locale: Locale }) {
  ...
  const bn = locale === "bn" ? SERVICE_BN[service.slug] : undefined;
  const features = bn?.features ?? en.features;
  const idealFor = bn?.idealFor ?? en.idealFor;
        ...
        <div className="container-x grid gap-10 lg:grid-cols-12">
          ...
          <Reveal delay={0.12} className="lg:col-span-4">
            <div className="card sticky top-24 p-7">
              <h2 className="text-h3">{t.services.interested}</h2>
        ...
        <section className="container-x py-10 md:py-14">
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {en.pricing.map((tier, i) => {
              const loc = locale === "bn" ? PRICING_BN[tier.name] : undefined;
              return (
              <Reveal key={tier.name} delay={i * 0.08} className={cn(tier.highlight && "order-first md:order-none")}>
                <div className={cn("card relative flex h-full flex-col overflow-hidden p-7", tier.highlight && "border-signal/60 bg-surface-2")}>
                  ...
                  <p className="mt-4 font-heading text-[44px] leading-none font-extrabold tracking-[-0.03em] tabular-nums">{loc?.price ?? tier.price}</p>
                  <dl className="mt-6 grid grid-cols-2 gap-3 border-y border-line py-4 text-[14px]">
                    <div>
                      <dt className="text-faint">{t.services.audio}</dt>
                      <dd className="mt-0.5 font-semibold">{loc?.audioCut ?? tier.audioCut}</dd>
                    </div>
                    <div>
                      <dt className="text-faint">{t.services.video}</dt>
                      <dd className="mt-0.5 font-semibold">{loc?.videoCut ?? tier.videoCut}</dd>
                    </div>
                  </dl>
                  <ul className="mt-5 space-y-2 text-[15px]">
                    {(loc?.features ?? tier.features).map((f) => (
          ...
          {others.map((s, i) => (
            <Reveal key={s.slug} delay={i * 0.06}>
              <Link href={`${b}/services/${s.slug}`} className="card card-hover group flex h-full flex-col p-6">
                ...
                <h2 className="text-h3 mt-5">{s[locale].title}</h2>
